        ...
        if (aStep0.n != aStep1.n) {
          ...
          if (aStep0.y == aStep1.y) {
            if (aStep0.dy > 0) {
              ...
              const aPair0 = new LedPair(aS0, aS1)
              aLedStore[aPI - 1].push(aPair0)
              const aT0 = thePaths[aStep0.i].path[aStep0.x + 1]
              const aT1 = thePaths[aStep1.i].path[aStep1.x + 1]
              //addLed(theCircuit, aT1, aT0)
              const aPair1 = new LedPair(aT1, aT0)
              aLedStore[aPI + 1].push(aPair1)
            }
          }
        }
      }
...
//
/**
 * Attempt to build a Circuit3b object.
 * @param {*} theVCount - number of V wires [1,VMAX]
 * @param {*} theECount - number of E Wires [1,EMAX]
 * @returns
 */
export function buildCircuit3b(theVCount = 16, theECount = 4) {
  ...
  try {
    ...
    console.log('Error buildCircuit3b ', theErr)
